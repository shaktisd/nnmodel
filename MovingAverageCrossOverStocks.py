...
window = 5
df['PCT_CHANGE_SIGNAL'] = np.where((pd.rolling_max(df['Close'], window).shift(-window) - df['Close']) / df['Close']   > 0.01  , 1, 0)
...
newdf.to_csv('IOC_TRAIN.csv', sep=',',header=False, index=False)
